...
router.post('/razorpay/order', async(req,res)=>{
  try{
    ...
    const rr=await fetch(`${apiBase}/orders`,{method:'POST',headers:{Authorization:`Basic ${auth}`,'Content-Type':'application/json'},body:JSON.stringify({amount:Math.round(n*100),currency:'INR',receipt:`${normalizedPurpose.toLowerCase()}_${user.id}_${Date.now()}`,notes:{userId:user.id,purpose:normalizedPurpose}})});
    ...
    await prisma.razorpayPayment.create({data:{userId:user.id,orderId:order.id,amount:n,purpose:normalizedPurpose,status:'CREATED'}});
...
router.post('/razorpay/verify', async(req,res)=>{
  try{
    ...
    if(payment.status==='CAPTURED') return res.json({success:true,alreadyProcessed:true,amount:payment.amount,purpose:payment.purpose});

    if(payment.purpose==='ID_CREATION'){
      const updated=await prisma.$transaction(async(tx)=>{
        await tx.razorpayPayment.update({where:{id:payment.id},data:{paymentId:razorpay_payment_id,status:'CAPTURED'}});
        const request=await tx.idCreationRequest.findFirst({where:{creatorId:String(userId),status:'PENDING'},orderBy:{createdAt:'desc'}});
        if(request) await tx.idCreationRequest.update({where:{id:request.id},data:{paymentStatus:'PAID'}});
        await tx.user.update({where:{id:String(userId)},data:{paymentStatus:'Paid'}});
        return request;
      });
      return res.json({success:true,purpose:'ID_CREATION',amount:payment.amount,requestId:updated?.id||null});
    }
    ...
    res.json({success:true,purpose:'WALLET_RECHARGE',...updated});
  }catch(e:any){res.status(500).json({error:e.message});}
});
    ...
    const body=Buffer.isBuffer(req.body)?JSON.parse(req.body.toString('utf8')):req.body; const event=body?.event; const entity=body?.payload?.payment?.entity; if(event==='payment.captured'&&entity?.order_id){
      ...
      if(p&&p.status!=='CAPTURED'){
        await prisma.$transaction(async(tx)=>{
          await tx.razorpayPayment.update({where:{id:p.id},data:{paymentId:entity.id,status:'CAPTURED'}});
          if(p.purpose==='ID_CREATION'){
            const request=await tx.idCreationRequest.findFirst({where:{creatorId:p.userId,status:'PENDING'},orderBy:{createdAt:'desc'}});
            if(request) await tx.idCreationRequest.update({where:{id:request.id},data:{paymentStatus:'PAID'}});
            await tx.user.update({where:{id:p.userId},data:{paymentStatus:'Paid'}});
          }else{
            await tx.user.update({where:{id:p.userId},data:{walletBalance:{increment:p.amount}}});
            await tx.walletTransaction.create({data:{userId:p.userId,type:'RAZORPAY',amount:p.amount,description:'Razorpay webhook wallet recharge',razorpayOrderId:p.orderId,razorpayPaymentId:entity.id,status:'SUCCESS'}});
          }
        });
      }
    }
