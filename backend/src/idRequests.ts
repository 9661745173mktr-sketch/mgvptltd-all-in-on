...
const CHILD_ROLES: Record<string, string[]> = {
  ADMIN: ['MASTER_DISTRIBUTOR', 'SUPER_DISTRIBUTOR', 'DISTRIBUTOR', 'RETAILER'],
...
router.post('/register', async (req, res) => {
  try {
    ...
    if (!cleanName || cleanPhone.length !== 10 || !cleanEmail || cleanPassword.length < 6) {
      return res.status(400).json({ error: 'Real name, valid 10-digit mobile, Gmail/email and password (6+ characters) are required.' });
    }
    if (!ROLE_FEES[normalizedRole]) return res.status(400).json({ error: 'Invalid partner role.' });
    if (String(paymentMethod || '').toLowerCase() === 'upi' && !String(utr || '').trim()) {
      return res.status(400).json({ error: 'UTR is required for UPI payment.' });
    }

    if (parentId) {
      const parent = await prisma.user.findUnique({ where: { id: String(parentId) } });
      if (!parent || parent.accountStatus !== 'Active') return res.status(403).json({ error: 'Parent account is not active.' });
    ...
    const existing = await prisma.user.findFirst({
      where: { OR: [{ email: cleanEmail }, { phone: cleanPhone }, ...(username ? [{ username: String(username).trim() }] : [])] },
    });
    ...
    const result = await prisma.$transaction(async tx => {
      ...
      const request = await tx.idCreationRequest.create({
        data: {
          creatorId: user.id,
          requestedRole: normalizedRole,
          applicantName: cleanName,
          applicantMobile: cleanPhone,
          applicantEmail: cleanEmail,
          username: user.username || `${normalizedRole.toLowerCase()}-${user.id.slice(0, 8)}`,
          passwordHash: user.password,
          amount,
          utr: String(utr || '').trim() || null,
          paymentStatus: String(paymentMethod || '').toLowerCase() === 'upi' ? 'PENDING_VERIFICATION' : 'PAYMENT_PENDING',
          status: 'PENDING',
        },
      });
    ...
    return res.status(201).json({
      success: true,
      message: 'ID creation request submitted. Admin verification is required before login.',
      user: safeUser,
