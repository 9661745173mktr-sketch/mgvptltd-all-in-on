...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { if (pathname !== '/admin/login' && !sessionStorage.getItem('admin_token')) router.replace('/admin/login'); setReady(true); }, [pathname, router]);
  if (!ready) return <div style={{ background: '#090d16', minHeight: '100vh' }} />;
  if (pathname === '/admin/login') return <>{children}</>;
  const menuItems = [
    ['Dashboard & Stats', '/admin/dashboard', '📊'], ['ID Approval Requests', '/admin/id-requests', '🪪'], ['Service Requests', '/admin/service-requests', '📥'], ['Wallet Load Requests', '/admin/wallet-requests', '💰'], ['Aadhaar Service Control', '/admin/aadhaar-requests', '🆔'], ['User Hierarchy', '/admin/user-hierarchy', '👥'], ['Wallet & Transactions', '/admin/wallet-transactions', '💳'], ['Master Services Control', '/admin/services-control', '⚙️']
  ];
  return <div style={{ display: 'flex', background: '#090d16', minHeight: '100vh', color: '#fff', fontFamily: 'Inter, sans-serif' }}>
    <aside style={{ width: 270, background: '#0f172a', borderRight: '1px solid #1e293b', display: 'flex', flexDirection: 'column', padding: 20, position: 'fixed', height: '100vh', boxSizing: 'border-box' }}>
      ...
      <nav style={{ display: 'flex', flexDirection: 'column', gap: 8, flex: 1 }}>{menuItems.map(([name, href, icon]) => <Link key={href} href={href} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 15px', borderRadius: 10, textDecoration: 'none', color: pathname === href ? '#fff' : '#94a3b8', background: pathname === href ? '#0369a1' : 'transparent', fontWeight: 600, fontSize: 14 }}><span>{icon}</span><span>{name}</span></Link>)}</nav>
