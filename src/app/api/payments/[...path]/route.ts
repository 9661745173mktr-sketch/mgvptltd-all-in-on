...
async function proxy(request: Request, path: string[]) {
  ...
  try {
    ...
    const response=await fetch(`${base}/payments/${path.map(encodeURIComponent).join('/')}${new URL(request.url).search}`,{method,headers,body:['GET','HEAD'].includes(method)?undefined:await request.arrayBuffer(),cache:'no-store'});
